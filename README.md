<!--
  GitHub Profile README — Malila Nyamai
  Markdown + minimal inline HTML (no CSS/JS)
-->

<div align="center">

  <h1>👋 Hi, I'm <strong>Malila Nyamai</strong></h1>
  <p><em>Full-stack Developer • Kenya</em></p>

  <p>
    Building impactful web applications with <strong>React</strong>, <strong>Node.js</strong>, and <strong>cloud technologies</strong>.<br>
    Passionate about clean UI, accessibility, and scalable architectures.
  </p>

  <p>
    <a href="https://www.linkedin.com/in/malila-nyamai-0b2711221/" target="_blank">LinkedIn</a> •
    <a href="https://malila-nyamai.netlify.app/" target="_blank">Portfolio</a> •
    <a href="https://github.com/joashnyamai" target="_blank">GitHub</a>
  </p>

</div>

---

### 💡 About
I'm **Malila Nyamai**, a dedicated software developer based in **Kenya**.  
I design and build impactful web applications focused on **excellent UX** and **scalable architectures**.

Active under [@joashnyamai](https://github.com/joashnyamai) contributing to open-source, mentoring, and helping teams adopt modern web technologies.

- 🔭 **Working on:** scalable, responsive front-end architectures  
- 🌱 **Learning:** TypeScript, DevOps, Docker  
- 👯 **Open to:** collaborations across East Africa  
- 💬 **Ask me about:** React, Node.js, and the Kenyan tech scene  
- 😄 **Pronouns:** he/him  
- ⚡ **Fun fact:** I built a weather dashboard that plays Benga on rainy days 🎶  

---

### 🧠 Focus Areas & Projects

#### Responsive Web Apps
Crafting modern, performant web experiences using React, accessibility best practices, and reusable design systems.

#### Front-End Experiences
Building smooth, interactive UIs with intuitive animations and component-driven architecture.

#### Cloud & Backend
Creating efficient, serverless, and containerized Node.js apps using CI/CD pipelines and AWS.

**Tech Stack:**

<div align="center">

![React](https://img.shields.io/badge/React-20232A?style=flat&logo=react&logoColor=61DAFB)
![TypeScript](https://img.shields.io/badge/TypeScript-007ACC?style=flat&logo=typescript&logoColor=white)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat&logo=nodedotjs&logoColor=white)
![Express](https://img.shields.io/badge/Express-000000?style=flat&logo=express&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-4EA94B?style=flat&logo=mongodb&logoColor=white)
![Postgres](https://img.shields.io/badge/PostgreSQL-316192?style=flat&logo=postgresql&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white)
![AWS](https://img.shields.io/badge/AWS-232F3E?style=flat&logo=amazon-aws&logoColor=white)
![CI/CD](https://img.shields.io/badge/CI%2FCD-181717?style=flat&logo=githubactions&logoColor=white)

</div>

---

### 🤝 Contact & Links

- 💼 [LinkedIn](https://www.linkedin.com/in/malila-nyamai-0b2711221/)  
- 🌐 [Portfolio](https://malila-nyamai.netlify.app/)  
- 🧑‍💻 [GitHub](https://github.com/malila-nyamai)  
- ✉️ Email: `nyamaibigjoash@gmail.com`

---

<p align="center">
  <small>Made with ❤️ — <em>Malila Nyamai • Kenya</em></small>
</p>
